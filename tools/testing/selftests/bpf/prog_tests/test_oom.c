	...
	for (;;) {
		...
		for (i = 0; i < ARRAY_SIZE(cgroups); i++) {
			...
			size = read_cgroup_file(cgroups[i].path,
						"memory.events",
						buf, sizeof(buf));
			if (!ASSERT_GT(size, 0, "read memory.events")) {
				ret = -1;
				break;
			}

			if (!ASSERT_OK_PTR(strstr(buf, "oom_kill 1"),
...
void test_oom(void)
{
	...
	link1 = bpf_map__attach_cgroup_opts(skel->maps.test_bpf_oom,
					    cgroups[0].fd, &opts);
	if (!ASSERT_OK_PTR(link1, "attach_oom_cgroup")) {
		link1 = NULL;
		goto cleanup;
	}
	...
	link2 = bpf_map__attach_cgroup_opts(skel->maps.test_bpf_oom,
					    root_fd, &opts);
	if (!ASSERT_OK_PTR(link2, "attach_oom_root")) {
		link2 = NULL;
		goto cleanup;
	}
